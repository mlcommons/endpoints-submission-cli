...
Run folder layout, as written by ``mlcommons/endpoints`` to its ``report_dir``
(see ``docs/endpoints-cli/reference/run-folder-layout.md``):
    ...
    <run_folder>/
        ...
        metrics/.ready                      — zero-byte completion sentinel

``performance/`` exists when the performance phase ran, ``accuracy/`` when the
accuracy phase ran; ``--mode both`` produces both.

This is the only layout accepted. Flat layouts that place ``result_summary.json``
or ``results.json`` at the top level are not supported.
"""
...
def build_archive(folder: Path, dest: Path | None = None, run_date: str | None = None) -> Path:
    ...
    (see :func:`truncate_responses`) to keep uploads small; the source folder on disk
    is never mutated.
    ...
    for results_path in sorted(folder.rglob(_ACCURACY_RESULTS_FILENAME)):
