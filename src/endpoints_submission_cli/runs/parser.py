...
Run folder layout expected by this parser:

    <run_folder>/
        system_info.json    — hardware/software metadata
...
def parse_run_folder(path: Path) -> dict[str, Any]:
    ...
    Args:
        path: Directory containing ``system_info.json``, ``config.yaml``,
    ...
    system_info = _load_json(path / "system_info.json")
